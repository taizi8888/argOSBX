...
# ==========================================
# 1. 环境依赖自检 (启用防死锁机制，恢复 GitHub 直连)
# ==========================================
check_env() {
    if [ ! -s "$FONT_FILE" ]; then
        echo " ⏳ 正在拉取海报渲染专用中文字体 (启用海外高速直连)..."
        mkdir -p "$FONT_DIR"
...
process_folder() {
    ...
    if [ "$NUM_FILES" -gt 0 ] && [ "$NEED_MAKE_TORRENT" = true ]; then
        ...
        if [ -n "$CUSTOM_PIECE_L" ]; then PIECE_L=$CUSTOM_PIECE_L; else
    ...
    if [ "$NEED_FFMPEG" = true ] && [ "$NUM_FILES" -gt 0 ] && [ -n "$MAIN_VIDEO" ]; then
        ...
        extract_screenshots() {
            ...
            local MAX_CONCURRENT=3; local current_jobs=0
            ...
            for (( i=0; i<$TOTAL_SHOTS; i++ )); do
                ...
                [ -z "$CUR_FILE" ] && CUR_FILE="${VIDEO_FILES[-1]}" && REL_TIME=$((fd - 5))
                ...
                (
                    if [ "$LAYOUT_TYPE" == "vr" ]; then
                        ...
                        if [ ! -f "$TMP_IMG_DIR/shot_$i.jpg" ]; then ffmpeg -nostdin -y -f lavfi -i color=c=black:s=2560x1440 -frames:v 1 "$TMP_IMG_DIR/shot_$i.jpg" >> "$LOG_FILE" 2>&1; fi
                    else
                        ...
                        if [ ! -f "$TMP_IMG_DIR/shot_$i.jpg" ]; then ffmpeg -nostdin -y -f lavfi -i color=c=black:s=1280x720 -frames:v 1 "$TMP_IMG_DIR/shot_$i.jpg" >> "$LOG_FILE" 2>&1; fi
                    fi
                ) &
            ...
            for (( i=0; i<$TOTAL_SHOTS; i++ )); do
                 if [ ! -f "$TMP_IMG_DIR/shot_$i.jpg" ]; then
                     if [ "$i" -gt 0 ]; then cp "$TMP_IMG_DIR/shot_$((i-1)).jpg" "$TMP_IMG_DIR/shot_$i.jpg"; else cp "$TMP_IMG_DIR/shot_1.jpg" "$TMP_IMG_DIR/shot_$i.jpg" 2>/dev/null; fi
                 fi
            done
        }
...
echo -e "\033[1;33m      PT 制种引擎 V5.1 (海外满血直连版)  \033[0m"
echo -e "\033[1;36m======================================\033[0m"
echo -e " \033[1;32m[1]\033[0m 自动模式 (全盘深度扫描与制种)"
echo -e " \033[1;32m[2]\033[0m 手动模式 (输入指定文件夹名称)"
echo -e " \033[1;32m[3]\033[0m 云端同步 (仅精准拉取更新本引擎)"
...
case $MODE in
    ...
    2) read -p " 请输入具体的文件夹名: " NAME; echo ""; process_folder "$NAME" ;;
    3) 
        # 移除代理，恢复甲骨文直连
        echo -e "\n\033[1;33m⏳ 正在从 GitHub 高速直连拉取核心单文件...\033[0m"
        curl -Ls --connect-timeout 10 -m 120 https://raw.githubusercontent.com/taizi8888/argOSBX/main/pt-webui/pt_make.sh | tr -d '\r' > /root/pt_make.sh && chmod +x /root/pt_make.sh && exec /root/pt_make.sh 
        ;;
    *) exit 0 ;;
esac
