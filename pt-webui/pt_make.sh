...
process_folder() {
    ...
    # 核心防御 1：将字符串写入文本文件，彻底避开单引号/冒号等危险字符注入
    HEADER_TEXT="文件名: $(basename "$MAIN_VIDEO")    大小: $FILE_SIZE    分辨率: $RESOLUTION    时长: $DUR_FORMAT"
    echo -e "$HEADER_TEXT" > "$TMP_IMG_DIR/header.txt"

    ffmpeg -nostdin -f lavfi -i color=c=black:s=2560x90 -vframes 1 -y "$TMP_IMG_DIR/header_bg.jpg" >/dev/null 2>&1
    
    # 核心防御 2：如果字体缺失或压制失败，直接拷贝纯黑底图顶替，防止最后合成崩溃
    ffmpeg -nostdin -y -i "$TMP_IMG_DIR/header_bg.jpg" -vf "drawtext=fontfile='$FONT_FILE':textfile='$TMP_IMG_DIR/header.txt':fontcolor=white:fontsize=36:x=40:y=(h-text_h)/2" "$TMP_IMG_DIR/header.jpg" >/dev/null 2>&1
    if [ ! -f "$TMP_IMG_DIR/header.jpg" ]; then
        cp "$TMP_IMG_DIR/header_bg.jpg" "$TMP_IMG_DIR/header.jpg"
    fi
    ...
    extract_screenshots() {
        ...
        # 核心防御 3：补齐缺失帧。如果某张图截取失败，生成对应尺寸的黑屏图顶替，严防拼图指令失败
        for (( i=0; i<$TOTAL_SHOTS; i++ )); do
...
clear
echo "======================================"
echo "      ArgOSBX Worker Engine V6.5      "
echo "    (真 2K 高速并发 + 钛合金自愈)     "
echo "======================================"
echo " [1] 全盘自动化扫描"
echo " [2] 指定文件夹手动制种"
echo " [3] 云端同步 (shdetai)"
